...
function updateCounts() {
    ...
    (document.querySelector('.pending-count') as HTMLElement).textContent = (allItems.length - completedItems.length).toString(); //미완료 항목 개수
    (document.querySelector('.completed-count') as HTMLElement).textContent = (completedItems.length).toString(); //완료 항목 개수
}
updateCounts(); //초기값
...
// 카테고리 업다운
const categoryToggleBtn = document.querySelector('.category .arrow-button') as HTMLElement
const categoryToggleBtnEl = document.querySelector('.category .arrow-button p')as HTMLElement
...
//카테고리 추가
const categoryAdd = document.querySelector('.category_add') as HTMLElement
const selectCategoryList = document.querySelector('.select-category-list') as HTMLElement

categoryAdd.addEventListener('click', function () {
    const categoryCount :number = document.querySelectorAll('.category-item2').length;
    const categoryAddItem: HTMLLIElement = document.createElement('li'); //li 요소 생성
    const categoryAddItem2: HTMLElement = document.createElement('li');//li 요소 생성
...
//카테고리 정렬
function categorySelect(this: HTMLElement) {
    (document.querySelector('.category-name') as HTMLElement).textContent = this.textContent;
    ...
    todoItems.forEach(item => {
        const itemCategory = (item.querySelector('.item-category') as HTMLElement).classList[1];
...
function categoryAll() {
    (document.querySelector('.category-name') as HTMLElement).textContent ='카테고리'
...
//'+추가' 버튼 누를 때
const makeTodoBtn = document.querySelector('.make-todo') as HTMLElement
const makeTodoBtnText = document.querySelector('.make-todo p') as HTMLElement
const makeTodoList = document.querySelector('.make-todo-list') as HTMLElement
...
// + 버튼 클릭 시
const addBtn = document.querySelector('.input_button') as HTMLElement;
...
const category2ToggleBtn = document.querySelector('.category-todo .arrow-button') as HTMLElement
const categoryList2 = document.querySelector('.category-list2') as HTMLElement
...
function categorySelect2 (this : HTMLElement) {
    (document.querySelector('.category-todo-name') as HTMLElement).textContent = this.textContent
...
//입력창 기본 설정
const inputArea = document.querySelector('.todo-text') as HTMLInputElement;
const dateArea = document.querySelector('.todo-date') as HTMLInputElement;
...
function todoList () {
    ...
    category.textContent = (document.querySelector(`.${selectCategoryName}`) as HTMLElement).textContent
    ...
    checkBtn.addEventListener('click', function() {
        ...
        // 맨 아래로 이동
        if(list.classList.contains('completed')) {
            (document.querySelector('.list-items') as HTMLElement).appendChild(list);
        }else {
            orderAddList(list);
        }
    ...
    deleteBtn.addEventListener('click', function() {
        (document.querySelector('.list-items') as HTMLElement).removeChild(list); 
...
function searchTodos () {
    ...
    const todoItems = document.querySelectorAll('.item')
    todoItems.forEach(item => {
        if((item.querySelector('.item-name') as HTMLElement) === null) {
            return ;
        }
        const itemText = (item.querySelector('.item-name') as HTMLElement).textContent!.toLowerCase();
...
//리스트 추가 순서
function orderAddList(list : HTMLElement) {
...
const calendarBtn = document.querySelector('.calendar-button') as HTMLElement;
const cancelBtn = document.querySelector('.cancel-button') as HTMLElement;
...
//캘린더
const monthText = document.querySelector('.calendar-header h2') as HTMLElement
...
navs.forEach(nav => {
    nav.addEventListener('click', event => {
        const btnId = (event.target as HTMLElement).id;
...
//날짜에 해당하는 리스트 보여주기
function dateSelect (this: HTMLElement) {
...
function dateMatchFunc() {
   ...
   const selectDate = (document.querySelector('.selected') as HTMLElement).textContent;
    let listHtml = '';

    let matchDate = `${year}-${String(month+1).padStart(2, '0')}-${String(selectDate).padStart(2, '0')}`
    const todoItems = document.querySelectorAll('.item')
    const dayTodo = document.querySelector('.day-todo') as HTMLElement
    todoItems.forEach(item => {
        const itemDate = (item.querySelector('.item-date') as HTMLElement).textContent;
        if (itemDate === matchDate) {
            const itemName = item.querySelector('.item-name')
            if(itemName && itemName.classList.contains('completed-text'))
            {
                listHtml += `<li class="completed-text">${itemName.textContent}&nbsp;&nbsp;<span style="color: #858B97;">${(item.querySelector('.item-category') as HTMLElement).textContent}</span></li>`
            }else if(itemName) {
                listHtml += `<li>${itemName.textContent}&nbsp;&nbsp;<span style="color: #858B97;">${(item.querySelector('.item-category') as HTMLElement).textContent}</span></li>`  
            }
        }
    });
...
function dateCheck() {
    ...
    haveTodoDate.forEach(date => {
        ...
        const dateArr = (date as HTMLElement).textContent!.split('-').map(num =>parseInt(num))
        if(dateArr[0] === year && dateArr[1] === (month+1)) {
          document.querySelectorAll('.date-wrap ul li').forEach(listDate => {
            if(listDate !== null) {
                if(dateArr[2] === parseInt((listDate as HTMLElement).textContent!)){
